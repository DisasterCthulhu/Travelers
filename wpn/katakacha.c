...
void katakacha_abjure_shield(object who, float force, object target, object shield) {
	...
	user_message(([
		Message_Content                         : ({
			({ 'a', Identity_Render_Name(identity, 0) }), "darts from", 0, "into", ({ 's', target, shield }),
		}),
...
void katakacha_abjure_protective_manifestation(object who, float force, object target, object manifestation) {
	...
	user_message(([
		Message_Content                         : ({
			({ 'a', Identity_Render_Name(identity, 0) }), "zigzags from", 0, "into", ({ 's', target, manifestation }),
		}),
		Message_Senses                          : Message_Sense_Astral | Message_Sense_Visual,
	]), who);
	if(semirandom(rating) > semirandom(force)) {
...
void katakacha_overwhelm_mortal_wound(object who, object target) {
	...
	user_message(([
		Message_Content                         : ({
			({ 'a', Identity_Render_Name(identity, 0) }), "spirals off the surface of", 0, "and surges into", ({ 's', target, "broken body" }),
		}),
		Message_Senses                          : Message_Sense_Visual | Message_Sense_Astral,
	]), who);
...
void katakacha_end_of_round_effects(mapping args) {
	unless(enhancement)
		return;
	object who = args["who"];
	unless(who)
	...
	if(query_any_realm("NM", who))
		return;
	object array targets = filter(who->query_attackers(), (: environment($1) == environment($2) :), who);
	unless(sizeof(targets))
		return;
	foreach(object target : targets) {
		if(who && target && !target->query_dying() && target->query_mortal_wound()) {
			if(!who->combat_mode()->query_combat_mode_nonhostile() && random(100) > target->query_resistance(enhancement)) {
	...
	if(!random(4) && weapon()->query_discharges()) {
		if(query_user() && query_user()->query_attacker() && !query_user()->query_attacker()->query_dying()) {
			weapon()->discharge();
			return;
		}
	}
	...
	if(!link)
		return;
	if(member(link->travelers_query_bestowals(), "katakacha abjuration") == Null)
		return;
		...
		foreach(object target : targets) {
			...
			foreach(object obj : target->query_wielded_weapons()) {
				if(obj->is_weapon() && obj->weapon()->type()->query_weapon_type_shield()) {
					...
					who->add_skill_exp(Skill_Abjuration, Learn_Rare);
					return;
				}
			}
		}
	...
	if(effective_communion > random(400)) {
		foreach(object target : targets) {
			foreach(object obj : target->query_armour_worn() + target->query_unbound_armour()) {
				if(obj->query_property(Prop_Manifestation) && !obj->query_indestructible()) {
					if(!who->check_spell_points(20.0))
						return;
					katakacha_abjure_protective_manifestation(who, effective_communion, target, obj);
					who->add_skill_exp(Skill_Abjuration, Learn_Rare * 2);
					return;
				}
			}
		}
...
void katakacha_user_do_fight(mapping args) {
	call_out("katakacha_end_of_round_effects", 1, args);
}
...
varargs string katakacha_clear_enhancement(status suppress_message) {
	...
	if(enhancement) {
		if(suppress_message)
			...
			user_message(([
				Message_Content                 : ({
					({ 't', Identity_Render_Name(katakacha_query_enhancement_identity(), 0) }), "flickering across the surface of", 0, "fade away",
				}),
				Message_Senses                  : Message_Sense_Visual | Message_Sense_Astral,
			]));
		enhancement = 0;
...
void katakacha_set_enhancement(object who, string array types) {
	...
	enhancement = types;
	mixed array message = ({});
	string new_motes = Identity_Render_Name(katakacha_query_enhancement_identity(), 0);
	if(existing_motes && existing_motes != new_motes)
		message += ({
			({ 't', existing_motes }), "flickering across the surface of", 0,
			"are interspersed with and then replaced by", new_motes,
		});
		...
		message += ({
			new_motes, "begin to flicker across the surface of", 0
		});
	user_message(([
