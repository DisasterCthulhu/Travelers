...
void ganesha_challenge_at_incarnate(object who) {
	call_out("ganesha_challenge_recalculate_obstacle", 2);
	who->remove_hook(At_Incarnate, #'ganesha_challenge_at_incarnate);
}

void ganesha_challenge_at_skill_update(mapping args) {
	if(!recalculating && find_call_out("ganesha_challenge_recalculate_obstacle") == Null)
		call_out("ganesha_challenge_recalculate_obstacle", 10);
}
