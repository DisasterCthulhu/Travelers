...
void configure() {
    ::configure();
    set_obstacle_name("magick proscription");
    ...
    set_obstacle_initialize_process((:
        ...
        if(field) {
            ...
            field->ganesha_interdiction_field_set_realms(({ "NM" }));
    ...
    set_obstacle_overcome_display(([
        Message_Content         : ({
            0, ({ "sense", 0 }), "the proscription against using magick being lifted from", ({ 'o', 0 }),
        }),
